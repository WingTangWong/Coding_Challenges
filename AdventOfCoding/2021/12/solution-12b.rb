...
DEBUG=false
# Change/difference for 12B:
#   * one small cave can be visited twice.
#   * All other rules the same.
# 
# So for each try... one random small cave has an extra weight added.
#
#
# Find all paths
#
...
def do_route( origin, map, route, weights )
  if origin == "start" then
    ...
    orig_map_weights=create_weights(map)

    # First pass, handle with things not modified
    map_weights = orig_map_weights.dup
    # process with that updated value
    if DEBUG then
      pp(map_weights)
    end

    # 2nd pass onward, only process with one small cave bumped up
    # Okay, here is the 12B specific bit: now that we have
    # a weight map, we need to try out solutions for each
    # case where one small cave can be visited twice.
    for mwk,mwv in orig_map_weights.dup do
      # Okay, loop through each weight key
      # And only act on cases where the 
      # item is a small cave node
      map_weights = orig_map_weights.dup
      if ( mwv == 1 ) & ( mwk != "start" ) then
        map_weights[mwk] += 1
...
# Okay, lets process through all of the maps
for target_map in WORKING_DATA_FILES do
  ...
  # Only answers ending with "end"
	answers = answers.map { |x| if x.include? "end" then x end} 
	answers.delete(nil)

	# Now, let's get rid of dups
  answers = answers.sort.uniq.to_a
