...
# Squid bingo.... pick last board to win.
#
# Squid Bingo....
#
#
  ...
  display_matrix(board_matrix)
	...
	for drawing in drawn_numbers do
      ...
      if DEBUG then
	    ...
	    if draws_to_win > max_win_steps then
        max_win_steps = draws_to_win
        max_board_score = board_score.clone
        max_last_drawn = drawing.to_i * 1
      end
...
max_final_board_score = max_board_score.to_i * max_last_drawn.to_i

puts ">> Min Draw#[#{min_win_steps}] :: Last Drawn [ #{last_drawn} ] :: Board Score [ #{min_board_score} ] :: Final Board Score [ #{final_board_score} ] "
puts ">> Max Draw#[#{max_win_steps}] :: Last Drawn [ #{max_last_drawn} ] :: Board Score [ #{max_board_score} ] :: Final Board Score [ #{max_final_board_score} ] "
