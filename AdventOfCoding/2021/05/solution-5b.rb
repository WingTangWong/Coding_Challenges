...
def plot_line( map, src, dst )
  ...
  if is_diagonal?( src,dst) then
    ...
    if src[0].to_i > dst[0].to_i then
      xarr=Range.new(dst[0].to_i,src[0].to_i).to_a.reverse
    else
      xarr=Range.new(src[0].to_i,dst[0].to_i).to_a
    end
    if src[1].to_i > dst[1].to_i then
      yarr=Range.new(dst[1].to_i,src[1].to_i).to_a.reverse
    else
      yarr=Range.new(src[1].to_i,dst[1].to_i).to_a
    end
    ...
    # Okay, now let's plot it out
    for i in 0..(xarr.count-1) do
      x=xarr[i]
      y=yarr[i]
      map[x,y] = map[x,y] + 1
      if DEBUG then
